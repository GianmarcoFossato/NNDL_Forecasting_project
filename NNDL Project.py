...
@app.cell
def _(Path, execution_mode, json, mo, os, subprocess):
    mo.stop(
        execution_mode.value != "Hyperparameter Tuning",
        mo.md("*Hyperparameter Tuning is currently disabled by the Control Panel selection.*")
    )

    print("Checking disk for existing tuning outputs...")

    # 1. Look for parameter files generated by tune.py inside test_results
    base_results_dir = Path("test_results")
    search_dir = base_results_dir / "tuning" if (base_results_dir / "tuning").exists() else base_results_dir
    json_candidates = list(search_dir.glob("**/best_params_*.json")) if search_dir.exists() else []

    # 2. Stop cleanly if no previous results exist on disk
    mo.stop(
        not json_candidates,
        mo.md("**No existing tuning results found on disk.** Please execute Cell 1 first to run hyperparameter search.")
    )

    # 3. Sort candidates by last modification time (most recent run first)
    json_candidates.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    target_json = json_candidates[0]

    print(f"Found existing tuning result: {target_json}")

    # 4. Safely load optimal parameters
    best_params = {}
    try:
        with open(target_json, "r", encoding="utf-8") as f:
            ...
            if isinstance(data, dict) and "best_params" in data and isinstance(data["best_params"], dict):
                ...
                print(f"Loaded {len(best_params)} optimal parameters from file.")
            else:
                print(f"Warning: '{target_json}' is missing a valid 'best_params' object. Falling back to shell script defaults.")
    except Exception as e:
        print(f"Error loading '{target_json}': {e}. Falling back to shell script defaults.")

    # 5. Map best parameters into uppercase environment variables
    env_vars = os.environ.copy()
    for param_key, param_value in best_params.items():
        env_vars[param_key.upper()] = str(param_value)

    # Run shell script
    hp_script_path = "scripts/long_term_forecast/Tuning/HyPT_opt.sh"
    if os.path.exists(hp_script_path):
        print(f"Starting evaluation with script '{hp_script_path}'...")
        subprocess.run(["chmod", "+x", hp_script_path])
        subprocess.run(["bash", hp_script_path], env=env_vars)
        print("Evaluation completed!")
    else:
        print(f"Error: Shell script '{hp_script_path}' was not found.")
    return
