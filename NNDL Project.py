...
def _(Path, execution_mode, json, mo, os, subprocess):
    ...
    # Look for parameter files generated by tune.py inside test_results
    ...
    # Stop cleanly if no previous results exist on disk
    mo.stop(
        not json_candidates,
        mo.md("**No existing tuning results found on disk.** Please execute Cell 1 first to run hyperparameter search.")
    )

    # Sort candidates by last modification time (most recent run first)
    ...
    # Safely load optimal parameters
    best_params = {}
    try:
    ...
    # Map best parameters into uppercase environment variables
